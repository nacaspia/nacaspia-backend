...
class News
{
    public  function scrape($limit = 100, $postLimit = 100)
    {
        ...
        $site = 'https://afsi.gov.az/az/media/xeberler';
            ...
            if ($url->successful()) {
                ...
                if (!empty($imagesUrl[0])) {
                    ...
                    if ($imageData === false) {
                        ...
                        // Dosya adını tanımla
                        $fileName = 'uploads/news/' . preg_replace('/\s+/', '-', mb_strtolower($title, 'UTF-8')) . '.' . $extension;
                ...
                if (!empty($imagesUrl[0])) {
                    foreach ($imagesUrl as $sliderUrl) {
                        ...
                        if ($sliderImageData === false) {
                            ...
                            $fileName = 'uploads/news/slider_image/' . $uniqueName . '.' . $extension;
    ...
    public static function dataSave($data)
    {
        ...
        try {
            if (!empty($data) && $data != NULL) {
                    ...
                    $newsTitle = ['az' => $title,'en' => '','ru' => ''];
                    $newsSlug = ['az' => Str::slug($title),'en' => '','ru' => ''];
                    $newsText = ['az' => $text,'en' => '','ru' => ''];
                    $newsFullText= ['az' => $fullText,'en' => '','ru' => ''];
                    $newsData = \App\Models\News::where('slug->az',$newsSlug)->first();
                    if (empty($newsData)) {
                        ...
                        $news->slug = $newsSlug;
                        $news->text = $newsText;
                        $news->fulltext = $newsFullText;
                        $news->image = $data['image'];
                        $news->slider_image = $data['slider_image'];

                        $news->datetime = date('Y-m-d H:i:s', strtotime($data['datetime']));
                        $news->status = 1;
                        $news->is_main = 0;
                        $news->order_by = 0;
                        $news->save();
                        $response = [
                            'success' => true,
                            'message' => 'Xeber bazaya yazıldı.',
                            'code' => 200
                        ];
                    }else{
                        $response = [
                            'success' => true,
                            'message' => 'Xeber bazada var.',
                            'code' => 200
                        ];
                    }

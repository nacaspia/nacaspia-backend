...
class Laboratory
{
    public  function scrape($limit = 100, $postLimit = 100)
    {
        ...
        $site = 'https://afsi.gov.az/az/laboratorys/mobil-laboratoriyalar/';
            ...
            if ($url->successful()) {
                ...
                if (!empty($imagesUrl[0])) {
                    ...
                    if ($imageData === false) {
                        ...
                        // Dosya adını tanımla
                        $fileName = 'uploads/laboratory/' . preg_replace('/\s+/', '-', mb_strtolower($title, 'UTF-8')) . '.' . $extension;
                ...
                if (!empty($imagesUrl[0])) {
                    foreach ($imagesUrl as $sliderUrl) {
                        ...
                        if ($sliderImageData === false) {
                            ...
                            $fileName = 'uploads/laboratory/slider_image/' . $uniqueName . '.' . $extension;
    ...
    public static function dataSave($data)
    {
        ...
        try {
            if (!empty($data) && $data != NULL) {
                ...
                $laboratoryTitle = ['az' => $title,'en' => '','ru' => ''];
                $laboratorySlug = ['az' => Str::slug($title),'en' => '','ru' => ''];
                $laboratoryText = ['az' => $text,'en' => '','ru' => ''];
                $laboratoryFullText= ['az' => $fullText,'en' => '','ru' => ''];
                $laboratory = new \App\Models\Laboratory();
                $laboratory->category_id = 6;
                $laboratory->city_id = 1;
                $laboratory->title = $laboratoryTitle;
                $laboratory->slug = $laboratorySlug;
                $laboratory->text = $laboratoryText;
                $laboratory->fulltext = $laboratoryFullText;
                $laboratory->image = $data['image'];
                $laboratory->slider_image = $data['slider_image'];
                $laboratory->datetime = date('Y-m-d H:i:s',strtotime($data['datetime']));
                $laboratory->status = 1;
                $laboratory->is_main = 0;
                $laboratory->order_by = 0;
                $laboratory->save();
                $response = [
                    'success' => true,

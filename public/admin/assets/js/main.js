(function ($) {
    ...
    $(document).ready(function () {
        ...
        $('#dirRtl').on('click', function () {
            ...
            $('html').attr('dir', 'rtl');
            $('#rtlStyle').attr('admin/assets/css/rtl-style.css');
        ...
        $('.color-palette').on('click', function () {
            ...
            $(this).addClass('active').siblings().removeClass('active');
            $('#primaryColor').attr('admin/assets/css/' + styleSheet + '.css');
        ...
        if (selectedStyleSheet) {
            ...
            $('#primaryColor').attr('admin/assets/css/' + selectedStyleSheet + '.css');
        }
        ...
        document.querySelectorAll('.editor').forEach(textarea => {
            ClassicEditor
                ...
                .then(editor => {
                    // Optional: Store the editor instance if needed
        ...
        if ($('.component-jquery-uploader').length) {
            ...
            let ajaxConfigMultiple = {
                ajaxRequester: function (config, uploadFiles, pCall, sCall, eCall) {
                    let progress = 0;
                    let totalFiles = uploadFiles.files.length;
                    let uploadedFiles = 0;
                    ...
                    let interval = setInterval(() => {
                        ...
                        if (progress >= 100) {
                            ...
                            for (let i = 0; i < totalFiles; i++) {
                                sCall({
                                    data: windowURL.createObjectURL(uploadFiles.files[i])
                                });
                            }
                        }
                    }, 300);
            ...
            $('#singleUpload').uploader({
                ajaxConfig: ajaxConfigSingle,
            });

            $('#multipleUpload').uploader({
                ajaxConfig: ajaxConfigMultiple,
                multiple: true,
            });
        }
...
*/
        ...
        if ($('#taskTable').length) {
            var dataTable = $('#taskTable').DataTable({
                ...
                "order": [[0, "desc"]], // 0, ilk sütun; "desc", azalan sırada başlat
            });
